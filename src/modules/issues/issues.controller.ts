...
const issuesGetController = async (req: Request, res: Response) => {
  ...
  res.status(200).json({ success: true, message: "Issue created successfully", data: result });
};
...
const issueUpdateController = async (req: Request, res: Response) => {
  const user = req.user;
  const {id}= req.params;
  const result = await issuesService.issuesUpdateIntoDb(req.body,id as string, user )
